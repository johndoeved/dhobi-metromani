...
var interestSchema = new import_mongoose.default.Schema({
...
var sessionSchema = new import_mongoose.default.Schema({
  id: { type: String, required: true, unique: true },
...
var reportSchema = new import_mongoose.default.Schema({
...
var adminNotificationSchema = new import_mongoose.default.Schema({
  id: { type: String, required: true, unique: true },
  type: String,
  message: String,
  targetId: String,
...
var adminSettingsSchema = new import_mongoose.default.Schema({ key: { type: String, index: true }, value: import_mongoose.default.Schema.Types.Mixed }, { strict: false });
...
if (process.env.MONGODB_URI) {
  import_mongoose.default.connect(process.env.MONGODB_URI, {
    maxPoolSize: 50,
    // Maintain up to 50 socket connections
    serverSelectionTimeoutMS: 5e3,
    // Keep trying to send operations for 5 seconds
    socketTimeoutMS: 45e3
    // Close sockets after 45 seconds of inactivity
  }).then(() => console.log("[MongoDB] Connected successfully, pooling configured")).catch((err) => console.error("[MongoDB] Error:", err));
...
<div class="c">
  <div class="h"><h1 class="t">\u{1F48D} Dhobi Metromani \u2014 Admin Alert</h1></div>
...
<div class="c">
  <div class="h"><h1 class="t">\u{1F48D} Dhobi Metromani</h1></div>
  <div class="b"><h3 style="color:#8B0000;">${title}</h3>${body}</div>
  ...
  app.post("/api/users/update", authenticateToken, (req, res) => {
    ...
    if (updated && targetUser) {
      ...
      if (req.user.role === "admin" && updates.status === "approved") {
        sendUserEmail(
          targetUser.email,
          "\u{1F389} Your Profile Has Been Approved! \u2014 Dhobi Metromani",
          userEmailHtml("Profile Approved!", `<p>Dear ${targetUser.name || "User"},</p><p>Congratulations! \u{1F389} Your profile on <strong>Dhobi Metromani</strong> has been <strong style="color:#006600;">approved and verified</strong>.</p><p>You can now log in and browse potential matches from the Dhobi community.</p>`),
          `Your profile has been approved! You can now log in and find matches.`
        );
      } else if (req.user.role === "admin" && updates.status === "blocked") {
        sendUserEmail(
          targetUser.email,
          "Your Profile Status Has Changed \u2014 Dhobi Metromani",
...
`;
  ...
  app.post("/api/auth/admin-forgot-password-otp", apiRateLimiter(5 * 60 * 1e3, 3, "Too many OTP requests. Please wait a few minutes."), async (req, res) => {
    ...
    const html = `<p>Your OTP for resetting the Admin Password is: <strong>${otp}</strong></p><p>This OTP is valid for 5 minutes. If you did not request this, please ignore this email.</p>`;
    const text = `Your OTP for resetting the Admin Password is: ${otp}

This OTP is valid for 5 minutes. If you did not request this, please ignore this email.`;
    sendUserEmail(email, "Dhobi Metromany - Admin Password Reset OTP", html, text);
