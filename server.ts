...
class UserDb {
  ...
  getAll(): MatrimonyUser[] {
    return this.users;
  }

  findByEmail(email: string): MatrimonyUser | undefined {
    return this.users.find(u => u.email.toLowerCase() === email.toLowerCase());
  }

  create(user: MatrimonyUser): void {
    this.users.push(user);
    this.save();
  }

  update(uid: string, updates: Partial<MatrimonyUser>): boolean {
...
// Rate limit request tracker: email -> array of timestamps
...
class NodemailerEmailProvider implements EmailProvider {
  ...
  async sendOtp(email: string, otp: string): Promise<boolean> {
    ...
    try {
      const debugDir = path.join(process.cwd(), '.gemini_debug');
      if (!fs.existsSync(debugDir)) {
        fs.mkdirSync(debugDir, { recursive: true });
      }
      fs.writeFileSync(path.join(debugDir, 'otp.txt'), otp, 'utf8');
...
async function startServer() {
  ...
  // Fetch all users list (syncs frontend and backend databases)
  ...
  app.post('/api/auth/send-otp', async (req, res) => {
    ...
    try {
      ...
      // Save/Overwrite OTP record in DB
      ...
      // Return success response (Do not return OTP in response; user retrieves via email or server console)
  ...
  app.post('/api/auth/verify-otp', async (req, res) => {
    ...
    try {
      ...
      otpDb.delete(cleanEmail); // Remove after success

      // User Authentication (Create or Login)
      let user = userDb.findByEmail(cleanEmail);
      let isNew = false;

      if (!user) {
        isNew = true;
        // Create basic user profile structure, which user completes during signup
        user = {
          uid: `u_${Date.now()}`,
          email: cleanEmail,
          status: 'pending',
  ...
  // Simulated Push Notification Dispatch API
